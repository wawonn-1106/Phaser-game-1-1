...
だいぶましな感じになった。物をインベントリの外でドロップしたら落とせるようにした。今は重なるけども

置けるものは家具、機械、柵、タイル、スプリンクラー、たいまつなど。
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
2月10日
方向転換→インベントリ常に表示しない、デコレーションモードに切り替えたら表示＋ものが置ける。体力→なしにする、経営に集中できるように。
素材を集めるのをプレイヤーではなくNPCに任せる→物量ごり押しで利益計算、経営戦略を考える必要が無くなってしまうから。
作業げーもいいけど、このゲームじゃなくてもいいよねってなるし。悲しいけど、個体値もなくす、得意不得意は村単位で再現、主人公の村は何が得意かも、
ランダムで決まる。二章での貿易で使う。加工はプレーヤーが担当。

デコレーションモードの実装、UIは左上にメニューのバー(三)、そのしたにデコレーションモード用のボタン。ぐらいに

口コミの実装、スクロール可能→形ができた。
同じ要領でランキングも作る。とりあえず部門は売上、口コミ、利益率とかでいいかな。グーグルの検索欄のタブみたいに切り替えできるように。
一つ一つは口コミと同じようにスクロールできるように。→ok

デコレーションモードを作る、モードにするとマップが五目状になってどこに物を置けるかが視覚的にわかる
左上のボタンで切り替え。
→マス目の表示に成功、一マスの大きさはTiledのと同じにする(今度). 
plcementPreview、ホットバーはデコレーションモードで表示するように改良。
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------
2月11日
今日はコードの整理中心で
明日はMongoDBでデータの上書きとかやってみる

整理のついでに、Profileの書き換え。個体値のグラフは消して、学生証みたいなUIにして、クリックしたら反転

今度は、適当なものを探してアニメーションの適用の練習してみる。
良い感じにできない。
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------
2月12日
MongoDBの接続→continueでセーブ前の座標に戻れるようにした。startは初期位置から。

マップ移動以来のきついエラーが出てる。
わからんや、nodeの勉強、真剣にしよ
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------
2月13日
DataはpreloadでダウンロードしてからTitleに飛ばすようにする
→セーブしてtitleに戻って再開したらゲーム立ち上げのPreload内から最新のデータを取得できなくて反映されない問題
→Titleでcontinue押されたときにDBから取得するように変更、（正直面倒だが
ゲッター初めて使った。かなり便利

マップ移動した後セーブしたらいい感じにならない、再会したら毎回初期位置からになる、sceneはあってる。

continueの時NPCが出現しない。わからんくて気分さがるから他のことする
統計アプリを導入、タイトルに戻るを設定の中に移動。

統計アプリ、タイトルに戻る、プロフィールの刷新をした。コミットメッセージ日本語でよくないか。
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------
2月14日
セーブとかはいい感じにできるようになった。
今日からUIを書き始める。

店（緑の家）の棚（焚火）でクリックしたら画面開いて、何を置くか、そして値段、品質を選ぶようにする。
↑なんか開いたら閉じれない問題発生。して中断

会話中に出た単語だけ辞書で見れるようにした。
開発中リアルタイムの尾道の天気知れるの便利でわらける

サーバーに接続できないエラー出たけど再起動したら治った。
再起動したらチームズ勝手につくのやめてほしい。
インベントリの取得、セーブをできるようにするのが明日の目標、それとどのアイテムを棚に置くか決めれるように

インベントリの取得、書き換え、置いたアイテムの配置のセーブも可能になった。
空のとこにアイテムを移動できないから、明日はそこから。
*/